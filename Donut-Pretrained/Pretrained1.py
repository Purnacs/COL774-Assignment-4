...
class DonutDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # inputs
        print(sample)
        img = Image.open("../Dataset/SyntheticData/images/"+sample["image"])
...
train_dataset = DonutDataset("../Dataset/SyntheticData/", max_length=max_length,
                             split="train", task_start_token="<s_cord-v2>", prompt_end_token="<s_cord-v2>",
                             sort_json_key=False, # cord dataset is preprocessed, so no need for this
                             )

# val_dataset = DonutDataset("../Dataset/SyntheticData/images", max_length=max_length,
#                              split="train", task_start_token="<s_cord-v2>", prompt_end_token="<s_cord-v2>",
#                              sort_json_key=False, # cord dataset is preprocessed, so no need for this
...
# %%
pixel_values, labels, target_sequence = train_dataset[1]
print(pixel_values)
print(labels)
...
print(labels.tolist()[:len(target_sequence)])
for id in labels.tolist()[:len(target_sequence)]:
  if id != -100:
    print(id)
